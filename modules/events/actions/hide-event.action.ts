/**
 * Server Action: hideEvent
 *
 * Oculta un evento del público (is_active = false).
 * Mantiene el evento para el productor pero no visible públicamente.
 *
...
interface HideEventResult extends EventResult {
  event?: Event;
}

export async function hideEvent(eventId: string): Promise<HideEventResult> {
  try {
    ...
    if (updateError || !updatedEvent) {
      return {
        ...
        error: {
          code: "UPDATE_ERROR",
          message: "Error al ocultar el evento",
        },
      };
    }
    ...
    console.error("hideEvent error:", error);
    return {
      success: false,
      error: {
        code: "INTERNAL_ERROR",
